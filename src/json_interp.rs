...
pub struct EnumeratedFold<S, RV = ()>(S, core::marker::PhantomData<RV>);

impl<S, RV> EnumeratedFold<S, RV> {
    pub const fn new(s: S) -> Self { EnumeratedFold(s, core::marker::PhantomData) }
}

impl<T, S: DynParser<T, Parameter = (u16, RV)>, RV: Debug + Summable<<S as ParserCommon<T>>::Returning>> ParserCommon<JsonArray<T>> for EnumeratedFold<S, RV> {
    type State = (u16, JsonArrayDropState<<S as ParserCommon<T>>::State, <S as ParserCommon<T>>::Returning>);
    type Returning = RV;
    fn init(&self) -> Self::State { (0, JsonArrayDropState::Start) }
}

impl<T, S: JsonInterp<T> + DynParser<T, Parameter = (u16, RV)>, RV: Copy + Debug + Summable<<S as ParserCommon<T>>::Returning>> JsonInterp<JsonArray<T>> for EnumeratedFold<S, RV> {
    #[inline(never)]
    fn parse<'a>(&self, state: &mut Self::State, token: JsonToken<'a>, destination: &mut Option<Self::Returning>) -> Result<(), Option<OOB>> {
        use JsonArrayDropState::*;
        use JsonToken::*;
        ...
        loop {
            match st {
                ...
                (Item(ref mut s, ref mut sub_destination), tok) => {
                    <S as DynParser<T>>::init_param(&self.0,(*ix,destination.ok_or(Some(OOB::Reject))?), s, sub_destination);
                    <S as JsonInterp<T>>::parse(&self.0, s, tok, sub_destination)?;
                    #[cfg(feature = "logging")]
                    trace!("destination {:?}", destination);
                    destination.as_mut().ok_or(Some(OOB::Reject))?.add_and_set(sub_destination.as_ref().ok_or(Some(OOB::Reject))?);
